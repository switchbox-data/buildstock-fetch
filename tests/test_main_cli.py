...
def test_interactive_mode(mock_checkbox, mock_select, mock_path):
    ...
    assert (
        "Result: resstock, 2021, tmy3, 1, ['CA', 'MA'], metadata, /workspaces/buildstock-fetch/test_output"
        in result.stdout
    )
    ...
    assert "Please select the release information and file type you would like to fetch:" in result.stdout
    assert (
        "Result: comstock, 2021, tmy3, 1, ['CA', 'TX'], ['15min_load_curve', 'metadata'], /workspaces/buildstock-fetch/test_output"
        in result.stdout
    )

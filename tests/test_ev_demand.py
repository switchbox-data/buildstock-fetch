...
def test_predict_num_vehicles_without_fitted_model(calculator, sample_metadata):
    """Test that error is raised when model is not fitted."""
    with pytest.raises(VehicleOwnershipModelError):
        calculator.predict_num_vehicles(sample_metadata)

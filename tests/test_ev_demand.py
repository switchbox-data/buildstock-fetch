...
def test_find_best_matches_skips_vehicle_tier_by_default(calculator):
...
def test_find_best_matches_with_vehicle_tier(calculator):
...
def test_sample_vehicle_profiles_uses_calculator_nhts_by_default(mock_nhts_data, mock_metadata, ev_ownership_df):
    ...
    profiles_explicit = EVDemandCalculator(**calculator_kwargs).sample_vehicle_profiles(
        mock_metadata,
        mock_nhts_data,
    )
    profiles_default = EVDemandCalculator(**calculator_kwargs).sample_vehicle_profiles(mock_metadata)
...
def test_generate_daily_schedules(calculator):
    ...
    schedules = calculator.generate_daily_trip_schedule(profile)
...
def test_normalize_day_trip_times_enforces_order_and_non_overlap(calculator):
    ...
    dep, arrival, keep = calculator._normalize_day_trip_times(departures, arrival_hours)
...
def test_generate_daily_schedules_no_invalid_or_overlapping_trips(calculator):
    ...
    schedules = calculator.generate_daily_trip_schedule(profile, rng=np.random.RandomState(0))
...
def test_generate_presence_schedules_marks_trip_hours_away(calculator):
    ...
    trip_schedules = calculator.generate_daily_trip_schedule(profile, rng=np.random.RandomState(0))
    presence = calculator.generate_presence_schedules(
        trip_schedules,
        vehicle_keys=[("b1", 1)],
    )[("b1", 1)]
...
def test_generate_soc_schedules_uses_prebuilt_presence(calculator):
    ...
    trip_schedules = calculator.generate_daily_trip_schedule(profile, rng=np.random.RandomState(0))
    presence_by_vehicle = calculator.generate_presence_schedules(
        trip_schedules,
        vehicle_keys=[("b1", 1)],
    )
    soc_kwargs = {
...
def test_off_peak_charging_blocks_between_trip_arrival_hours(calculator):
    from datetime import date

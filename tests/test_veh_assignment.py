...
@pytest.fixture
def model():
    """Create VehicleOwnershipModel instance for testing."""
    return VehicleOwnershipModel(max_vehicles=2, random_state=42)


def test_fit_vehicle_ownership_model_success(model, sample_pums_data):
    """Test successful model fitting."""
    fitted = model.fit(sample_pums_data)

    # Check that model was fitted
    assert fitted is not None
    assert model.vehicle_ownership_model is not None
    ...
    assert "metro" in model.label_encoders


def test_fit_vehicle_ownership_model_caps_vehicles_at_2(model):
    ...
    assert set(model.target_encoder.classes_) == set(expected_classes)


def test_fit_vehicle_ownership_model_with_sample_weights(model):
    ...
    assert fitted.coef_ == approx(expected_coef, rel=1e-6)


def test_fit_vehicle_ownership_model_handles_missing_values(model, caplog):
...
def test_predict_num_vehicles_success(model, sample_pums_data, sample_metadata):
    """Test successful vehicle prediction."""
    # First fit the model
    model.fit(sample_pums_data)

    # Then predict
    result_df = model.predict(sample_metadata)
...
def test_predict_num_vehicles_without_fitted_model(model, sample_metadata):
    """Test that predict raises when the model has not been fitted."""
    with pytest.raises(ValueError, match="has not been fitted"):
        model.predict(sample_metadata)


def test_predict_num_vehicles_missing_required_features(model, sample_pums_data):
    ...
    with pytest.raises(ValueError) as exc_info:
        model.predict(incomplete_metadata)
...
def test_predict_num_vehicles_consistent_predictions(model, sample_pums_data, sample_metadata):
    """Test that predictions are consistent for same input."""
    model.fit(sample_pums_data)

    # Make predictions twice
    result1 = model.predict(sample_metadata)
    result2 = model.predict(sample_metadata)

    # Results should be identical
    assert result1.equals(result2)


def test_predict_num_vehicles_feature_encoding(model, sample_pums_data):
    ...
    result = model.predict(new_metadata)
    assert len(result) == 1
    assert "vehicles" in result.columns


def test_predict_num_vehicles_feature_scaling(model, sample_pums_data):
    ...
    result = model.predict(extreme_metadata)
    assert len(result) == 2
    assert "vehicles" in result.columns
...
def test_random_seed_reproducibility(model, sample_pums_data, sample_metadata):
    ...
    result1 = model.predict(sample_metadata)
    result2 = model.predict(sample_metadata)
    result3 = model.predict(sample_metadata)

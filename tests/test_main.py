...
def test_fetch_annual_load_curve(cleanup_downloads):
    ...
    assert len(failed_downloads) == 0
    assert Path(
        f"data/{bldg_ids[0].get_release_name()}/load_curve_annual/state={bldg_ids[0].state}/upgrade={str(int(bldg_ids[0].upgrade_id)).zfill(2)}/{str(bldg_ids[0].bldg_id)!s}-{int(bldg_ids[0].upgrade_id)!s}.parquet"
    ).exists()

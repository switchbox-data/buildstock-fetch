...
from buildstock_fetch.utils import get_census_division_for_state, load_metadata, load_nhts_data, load_pums_data, load_metro_puma_map, load_weather_data
...
def test_ev_demand_config_metadata_path_property(test_config):
  """Test that metadata_path property is correctly constructed."""
  
  # Get the expected path based on the actual package structure
  expected_path = Path(__file__).parent.parent / "buildstock_fetch" / "data" / "resstock_tmy3_release_1" / "metadata" / "NY" / "metadata.parquet"
  
  assert test_config.metadata_path == str(expected_path)

...
DEFAULT_MAX_DEPARTURE_HOUR = 23
DEFAULT_MAX_ARRIVAL_HOUR = 24
...
class TripScheduleGenerator:
    ...
    def generate_daily_trip_schedule(
        ...
        for day_offset in range(days):
            ...
            miles_variance = rng.normal(
                selected_base_miles,
                selected_base_miles * self.miles_noise_std_fraction,
            )
            ...
            departures_with_variance = np.clip(
                selected_departures + departure_offsets, 0, self.max_departure_hour
            )
            arrival_hours_with_variance = np.clip(
                selected_arrivals + arrival_offsets, 1, self.max_arrival_hour
            )

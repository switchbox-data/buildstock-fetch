...
class DownloadedDataInfo:
    base_path: Path | S3Path  # note that this does not cast into Path or S3Pathif called with base_path as a str
    filename: str
    ...
    @classmethod
    def from_file_path(cls, path: Path | S3Path) -> Self:
        posix_path = path.as_posix() if isinstance(path, Path) else path.as_uri()
        ...
        base_path = S3Path(cast(str, groups["base_path"])) if is_s3_path(path) else Path(cast(str, groups["base_path"]))
    ...
    @property
    def file_path(self) -> Path | S3Path:
        return (
            self.base_path
            / self.release_key
            / self.file_type
...
def _fast_is_dir(path: Path | S3Path) -> bool:
    if path.suffix == ".parquet":  # shortcircuit for speed
        return False
    return path.is_dir()
...
def filter_downloads(
    ...
    return (
        ...
        if _fast_is_dir(release_key_path)
        ...
        if _fast_is_dir(file_type_path)
        ...
        if _fast_is_dir(upgrade_path)
        for file_ in upgrade_path.iterdir()
        if not _fast_is_dir(file_)
    )

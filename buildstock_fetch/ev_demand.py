...
class EVDemandCalculator:
    ...
    def predict_num_vehicles(self, metadata_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        if self.vehicle_ownership_model is None:
            raise ValueError("Vehicle ownership model not fitted")
    ...
    def sample_vehicle_profiles(self, metadata_df: Optional[pd.DataFrame] = None) -> Dict[Tuple[int, int], VehicleProfile]:
        """
        ...
        if self.nhts_df is None:
            raise ValueError("NHTS data not loaded")
        ...
        for (building_id, vehicle_id), profile in profile_params.items():
            ...
            schedule = TripSchedule(
                ...
                avg_temp_while_away=self.get_avg_temp_while_away(
                    profile.weekday_departure_hour, 
                    profile.weekday_arrival_hour, 
            ...
            schedule.kwh_consumed = self.miles_to_kwh(
                schedule.miles_driven, 
                schedule.avg_temp_while_away
            )
    ...
    def assign_battery_capacity(self, daily_kwh: pd.Series) -> pd.Series:
        ...
        for required in required_capacity:
            ...
            if len(suitable_batteries) > 0:
                assigned_capacities.append(suitable_batteries.iloc[0])
            else:
                # If no battery is large enough, assign the largest available
                assigned_capacities.append(battery_capacities.iloc[-1])
...
if __name__ == "__main__":
    from pathlib import Path
    from main import fetch_bldg_data, BuildingID

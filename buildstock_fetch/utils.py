...
def get_census_division_for_state(state: str) -> int:
    ...
    raise ValueError(f"State {state} not found in any census division")
...
def load_metadata(metadata_path: str) -> pl.DataFrame:
    ...
    metadata_df = metadata_df.with_columns([
        pl.when(pl.col("occupants") == "10+")
          ...
          .alias("occupants_temp")
    ])
    
    # Now cast to numeric
    metadata_df = metadata_df.with_columns([
        pl.col("occupants_temp").cast(pl.Int64).alias("occupants")
    ]).drop("occupants_temp")
    ...
    metadata_df = metadata_df.with_columns([
        pl.col("puma").str.slice(-5).alias("puma")
    ])
...
def load_nhts_data(nhts_path: str, state: str) -> pl.DataFrame:
    ...
    if not Path(nhts_path).exists():
        raise FileNotFoundError(f"NHTS file not found: {nhts_path}. Please run `just download-nhts` to download the data.")
...
def load_pums_data(pums_path: str, metadata_path: str = None) -> pl.DataFrame:
    """
    Load and preprocess the PUMS data for a specific state.
    
    Args:
        pums_path: Path to the PUMS data file
        metadata_path: Path to metadata file for metro-PUMA mapping (optional)
    """
    if not Path(pums_path).exists():
        raise FileNotFoundError(f"PUMS file not found: {pums_path}. Please run `just download-pums` to download the data.")
...
def load_metro_puma_map(metadata_path: str) -> pl.DataFrame:
    """
    Load the metro-pums mapping file. We need to assign the metro variable to the PUMS data based on a lookup of the puma code.
    """
    if not Path(metadata_path).exists():
        raise FileNotFoundError(f"Metadata file not found: {metadata_path}")
    
    metro_lookup_df = pl.read_parquet(metadata_path, columns=[
                                  'in.puma_metro_status', 'in.puma'])
    ...
    metro_lookup_df = metro_lookup_df.with_columns([
        pl.col("puma").str.slice(-5).alias("puma")
    ]).unique()
